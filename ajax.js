    ...
    if ($ajaxType == 'ajax') {
        ...
        xhr.onreadystatechange = () => {
            if (xhr.readyState == 4 && xhr.status == 200) {
                ...
                books = JSON.parse(xhr.responeText);
            ...
            <h1>${message}</h1>
             <form id="form-get-delete" onsubmit="return false">
                <div class="form-controls">
                    ...
                    <input name="id" id="id">
                    <p>(Leave blank to retrieve all records)</p>
                </div>
                <div class="form-controls">
                    <button id="go-get-delete">Start</button>
                </div>
            </form>
    `
    );
...
function postPutForm(type) {
    ...
    <h1>${message}</h1>
            ...
            <div class="form-controls">
                <button id="go-${type.toLowerCase()}">Start</button>
            </div>
        </form> 
    `
    );
    ...
    books.forEach(book => {
        str += `
            <tr>
                ...
                <td>${book.year}</td>
                <td>
                    <button onclick="prepareUpdate(${books.id})">Edit</button>
                    <button onclick="prepareDelete(${books.id})">Delete</button>
                </td>
            </tr>
        `
    });
...
function prepareUpdate(id) {
    ...
    $("#go-put").click(() => {
        let newBook = buildBookOnject();
        processAJAX('PUT', newBook.id.newBook);
    })
}

//Build book object
function buildBookOnject() {
    return {
